...
if [ ${STAGE} -le 8 ] && [ ${STOP_STAGE} -ge 8 ]; then
  ...
  FASTSPEECH2_CKPT_PATH=$FASTSPEECH2_DIR/log/ckpt/fastspeech2_200.ckpt                     # path to fastspeech2 checkpoint file
...
if [ ${STAGE} -le 9 ] && [ ${STOP_STAGE} -ge 9 ]; then
  ...
  EPOCH=200                              # Number of epoch for training hifigan
  CUDA_VISIBLE_DEVICES=1 python wetts/bin/hifigan_train.py train \
      --num_workers 32 \
