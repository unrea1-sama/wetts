...
from wetts.models.am.fastspeech2_emo.module import loss, emotion_emb_net
from wetts.models.am.fastspeech2_emo.module.dataset import (
...
def train(epoch, models, data_loader, loss_fn, optimizer, lr_scheduler,
          summary_writer):
    global train_step
    model, emo_net_dec = models
    model.train()
    emo_net_dec.train()
    for (keys, _, speakers, durations, text, mel, pitch, energy, text_length,
         mel_length, token_types, _, _, _, emotion) in data_loader:
        optimizer.zero_grad()
        ...
        (mel_prediction, postnet_mel_prediction, mel_mask, mel_len,
         ...
         enc_output_mask, emotion_emb,
         emotion_logit) = model(text,
                                text_length,
                                token_types,
                                mel,
                                mel_length,
        ...
        (duration_loss, pitch_loss, energy_loss, mel_loss, postnet_mel_loss,
         emotion_loss_enc, emotion_loss_dec, emotion_sty_loss) = loss_fn(
             ...
             energy, energy_prediction, enc_output_mask, mel, mel_prediction,
             postnet_mel_prediction, mel_mask, emotion_emb, dec_emotion_emb,
             emotion_logit, dec_emotion_logit, emotion)
        total_loss = (duration_loss + pitch_loss + energy_loss + mel_loss +
                      postnet_mel_loss + emotion_loss_enc + emotion_loss_dec +
                      emotion_sty_loss)
        total_loss.backward()
        ...
        summary_writer.add_scalar('encoder emotion classification loss',
                                  emotion_loss_enc.item(), train_step)
        summary_writer.add_scalar('decoder emotion classification loss',
                                  emotion_loss_dec.item(), train_step)
        summary_writer.add_scalar('emotion style loss',
                                  emotion_sty_loss.item(), train_step)
        print(("epoch {}, train step {}, train duration loss {}, "
               "train pitch loss {}, train energy loss {}, train mel loss {}, "
               "train postnet mel loss {}, enc emo cls loss: {}, "
               "dec emo cls loss: {}, emo sty loss: {}, "
               "train total loss: {}").format(epoch, train_step,
                                              duration_loss.item(),
                                              pitch_loss.item(),
                                              energy_loss.item(),
                                              mel_loss.item(),
                                              postnet_mel_loss.item(),
                                              emotion_loss_enc.item(),
                                              emotion_loss_dec.item(),
                                              emotion_sty_loss.item(),
                                              total_loss.item()))
        optimizer.step()
        lr_scheduler.step()
        train_step += 1
...
def main(args):
    ...
    (train_dataset, _, pitch_stats, energy_stats, phn2id,
     spk2id) = FastSpeech2TrainingDataset(args.train_data_list,
                                          ...
                                          args.phn2id_file, args.emo2id_file,
    ...
    model = model.cuda()
    emo_net_dec = emotion_emb_net.EmotionEmbeddingNetwork(
        80, [32, 32, 64, 64, 128, 128], 3, 2, 128, 128, [128, 256, 256],
        5).cuda()

    loss_fn = loss.FastSpeech2Loss()

    optimizer = Adam(
        itertools.chain(model.parameters(), emo_net_dec.parameters()), 1,
        conf.optimizer.betas, conf.optimizer.eps)
    ...
    for epoch in range(last_epoch + 1, args.epoch + 1):
        train(epoch, [model, emo_net_dec], train_data_loader, loss_fn,
