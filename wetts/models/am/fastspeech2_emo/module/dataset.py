...
def padding_training_samples(data):
    ...
    for sample in data:
        ...
        sorted_speaker_id = torch.tensor(
            [sample[i]['speaker_id'] for i in order], dtype=torch.int32)
        sorted_speaker = [sample[i]['speaker'] for i in order]
        sorted_emotion_id = torch.tensor(
            [sample[i]['emotion_id'] for i in order], dtype=torch.long)
        sorted_emotion = [sample[i]['emotion'] for i in order]
        sorted_duration = [
            torch.tensor(sample[i]['duration'], dtype=torch.int32)
            for i in order
        ]
        sorted_text_id = [
            torch.tensor(sample[i]['text_id'], dtype=torch.int32)
            for i in order
        ]
        sorted_text = [sample[i]['text'] for i in order]
        ...
        sorted_text_length = torch.tensor(
            [len(sample[i]['text_id']) for i in order], dtype=torch.int32)
        ...
        padded_duration = pad_sequence(sorted_duration,
                                       batch_first=True,
                                       padding_value=0)
        padded_text = pad_sequence(sorted_text_id,
                                   batch_first=True,
                                   padding_value=0)
        ...
        yield (sorted_keys, sorted_speaker, sorted_speaker_id, padded_duration,
               padded_text, padded_mel, padded_pitch, padded_energy,
               sorted_text_length, sorted_mel_length, padded_token_types,
               sorted_wav, sorted_text, sorted_emotion, sorted_emotion_id)
    ...
    for sample in data:
        ...
        for token in sample['text']:
            if token in lexicon:
                ...
                raise ValueError(
                    'Token {} not in lexicon or special tokens!'.format(token))
        sample['text'] = new_text
        yield sample
...
def parse_raw(data):
    ...
    for sample in data:
        ...
        sample['language'] = obj['language']
        sample['emotion'] = obj['emotion']
        sample['original_text'] = obj['original_text']
        yield sample
...
def CmvnDataset(data_list_file, conf):
    ...
    dataset = utils.Processor(dataset, parse_raw)
...
def FastSpeech2TrainingDataset(data_list_file, batch_size, spk2id_file,
    ...
    dataset = utils.Processor(dataset, processor.apply_emo2id, emo2id)

    # mel is not normalized
